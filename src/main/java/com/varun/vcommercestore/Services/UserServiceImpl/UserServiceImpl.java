...
public class UserServiceImpl implements UserServices {
    ...
    // =========================
    @Override
    public UserResponseDto updateUser(UserUpdateRequestDto userreqdto, String userId) {
        ...
        // check first, while user still has the OLD name
        if (!user.getUserName().equals(userreqdto.getUserName())
                && userRepository.existsByUserName(userreqdto.getUserName())) {
            throw new DataIntegrityViolationException("Account with Username already exists!");
        }

        user.setUserName(userreqdto.getUserName());
        user.setUserGender(userreqdto.getUserGender());
        user.setUserAddress(userreqdto.getUserAddress());
        user.setUserAbout(userreqdto.getUserAbout());
    ...
    // =========================
    @Override
    public PageResopnse<UserResponseDto> getAllUsers(int pagenumber, int pagesize, String sortby, String order) {

        logger.info("Fetching users. Page: {}, Size: {}, SortBy: {}, Order: {}", pagenumber, pagesize, sortby, order);
        ...
        return helper.getPageResponse(page, UserResponseDto.class);
    }
    ...
    public String saveUserProfileImage(String userid, String userImage) {
        logger.info("Saving profile image '{}' for user '{}'", userImage, userid);
        ...
        logger.info("Profile image '{}' saved successfully for user '{}'", userImage, userid);
        return userImage;
    }
    ...
    public String getProfileImagename(String userid) {
        logger.info("Fetching profile image name for user: {}", userid);
        ...
        String imageName = user.getProfileImage();
        logger.debug("Profile image for user '{}' is '{}'", userid, imageName);
        return imageName;
    }
    ...
    // DTO → ENTITY
    // =========================
    private User dtoToEntity(UserRequestDto userreqdto) {

        logger.debug(
                "Converting user DTO to User entity"
        );

        return mapper.map(userreqdto, User.class);
    }
    // =========================
    // ENTITY → DTO
    // =========================
    private UserResponseDto entityToDto(User savedUser) {
